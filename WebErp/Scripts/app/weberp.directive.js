(function (window, angular) {
	...
	app.directive('wSidebar', ['$log', '$timeout', function ($log, $timeout) {
		return {
            ...
            '<ul class="nav side-menu"><li ng-repeat="menu in section.menus"><a tooltip-placement="right" uib-tooltip="{{menu.tooltip | w-translate}}"><i class="fa fa-{{menu.icon}}"></i>{{menu.text}}<span class="fa fa-chevron-down"></span></a><w-menu-item ></w-menu-item></li></ul></w-menu-section></div>',
	...
	app.directive('wMenuSubitem', ['$log', '$compile', function ($log, $compile) {
		return {
			...
			link: function ($scope, $element, $attrs) {
				var elt = angular.element('<a uib-tooltip="{{child.tooltip| w-translate}}" tooltip-placement="right">{{child.text | w-translate}}<span ng-if="child.label" class="label label-{{child.label.variation}} pull-right">{{child.label.text}}</span></a>');
	...
	app.directive('wTopbar', [function () {
		return {
			...
			replace: true,
			transclude: true,
			template: '<div class="top_nav"><div class="nav_menu"><nav class="" role="navigation"><div class="nav toggle"><w-fa-bars  tooltip-placement="right" uib-tooltip="{{\'toggle-menu-tooltip\' | w-translate}}"></w-fa-bars></div><ul class="nav navbar-nav navbar-right" ng-transclude></ul></nav></div></div>',
		};
	}]);
	...
	app.filter('wTranslate', ['$filter', function ($filter) {
		return function (content) {
			var filter = $filter('TranslateFilter');
			if (filter)
